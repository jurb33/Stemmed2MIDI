...
#layout of the window
    layout = [
        ...
        [sg.Text("Error: No file to process or fields not filled!", visible=False, key='-ERROR-TEXT-')]
    ]
    ...
    while True:
            ...
            #TODO send file to be processed and fed to Audio2Midi
                ...
                if not values['-FILES-'] or values['-STEM-'].find(" ") != -1 or values['-NN-'].find(" ") != -1:
                    window['-ERROR-TEXT-'].update(visible=True)
                else:
                    window['-ERROR-TEXT-'].update(visible=False)
                    split_audio_file(values['-FILES-'])
    ...
    window.close()
#TODO send file to be processed and fed to
